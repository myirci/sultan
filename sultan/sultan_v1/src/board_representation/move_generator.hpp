...
class MoveGenerator
{
    ...
    std::pair<std::vector<Attack>, std::vector<Pin>> compute_attacks_and_pins(int8_t target, int8_t ntm) const;
    bool is_under_attack(int8_t sq, int8_t ntm) const;
    int8_t get_pin_direction(int8_t sq, std::vector<Pin> const & pins) const;
    void generate_king_moves(int8_t king_pos, int8_t ntm, bool under_check, std::vector<Move>& moves) const;
