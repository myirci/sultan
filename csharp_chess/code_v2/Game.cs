...
namespace Deneme
{
    ...
    public class Game
    {
        ...
        public void MakeMove(Move mv)
        {
            ...
            Hash ^= Zobrist.PieceKeys[(int)movingPiece][Zobrist.VisitOrder[(int)mv.From]]; 
            ...
            if (mv.Type == MoveType.Promotion || mv.Type == MoveType.PromotionAndCapture)
            {
                ...
                Hash ^= Zobrist.PieceKeys[(int)mv.Promoted][Zobrist.VisitOrder[(int)mv.To]];
            }
            else
            {
                GameBoard.BoardArray[(int)mv.To] = movingPiece;
                movingSide.UpdatePiecePosition(mv.From, mv.To, movingPiece);
                Hash ^= Zobrist.PieceKeys[(int)movingPiece][Zobrist.VisitOrder[(int)mv.To]];
            }

            if (mv.Type == MoveType.Capture || mv.Type == MoveType.PromotionAndCapture)
            {
                movedSide.RemovePiece(mv.To, mv.Captured);
                Hash ^= Zobrist.PieceKeys[(int)mv.Captured][Zobrist.VisitOrder[(int)mv.To]];
            } 
            ...
            if (mv.Type == MoveType.EnPassantCapture)
            {
                ...
                Hash ^= Zobrist.PieceKeys[(int)mv.Captured][Zobrist.VisitOrder[sq]];
            }
            else if (mv.Type == MoveType.KingSideCastle)
            {
                GameBoard.BoardArray[(int)movingSide.KSCastlingSquares[3]] = Piece.eP;
                GameBoard.BoardArray[(int)movingSide.KSCastlingSquares[1]] = movingSide.Rook;
                movingSide.UpdatePiecePosition(movingSide.KSCastlingSquares[3], movingSide.KSCastlingSquares[1], movingSide.Rook);
            }
            else if (mv.Type == MoveType.QueenSideCastle)
            {
                GameBoard.BoardArray[(int)movingSide.QSCastlingSquares[4]] = Piece.eP;
                GameBoard.BoardArray[(int)movingSide.QSCastlingSquares[1]] = movingSide.Rook;
                movingSide.UpdatePiecePosition(movingSide.QSCastlingSquares[4], movingSide.QSCastlingSquares[1], movingSide.Rook);
            }
            ...
            {
                if (mv.From == movingSide.QSCastlingSquares[4])      QSCastle(movingSide.Color, false);
                else if (mv.From == movingSide.KSCastlingSquares[3]) KSCastle(movingSide.Color, false);
            }
            
            if (mv.Captured == movedSide.Rook)
            {
                if (mv.To == movedSide.QSCastlingSquares[4])      QSCastle(movedSide.Color, false);
                else if (mv.To == movedSide.KSCastlingSquares[3]) KSCastle(movedSide.Color, false);
            }
        ...
        public void UnMakeMove()
        {
            ...
            else if (mv.Type == MoveType.KingSideCastle)
            {
                ...
                movedSide.UpdatePiecePosition(movedSide.KSCastlingSquares[1], movedSide.KSCastlingSquares[3], movedSide.Rook);
                GameBoard.BoardArray[(int)movedSide.KSCastlingSquares[1]] = Piece.eP;
                GameBoard.BoardArray[(int)movedSide.KSCastlingSquares[3]] = movedSide.Rook;
            }
            else if (mv.Type == MoveType.QueenSideCastle)
            {
                movedSide.UpdatePiecePosition(mv.To, mv.From, movedSide.King);
                ...
                movedSide.UpdatePiecePosition(movedSide.QSCastlingSquares[1], movedSide.QSCastlingSquares[4], movedSide.Rook);
                GameBoard.BoardArray[(int)movedSide.QSCastlingSquares[1]] = Piece.eP;
                GameBoard.BoardArray[(int)movedSide.QSCastlingSquares[4]] = movedSide.Rook;
            }
            else if (mv.Type == MoveType.Promotion)
            {
                movedSide.RemovePiece(mv.To, mv.Promoted);
